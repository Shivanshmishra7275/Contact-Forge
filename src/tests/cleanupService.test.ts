 ...
 *
 * scanContactForIssues is a pure function (given a contact object and phones/emails arrays),
...
describe('scanContactForIssues — missing_name', () => {
  it('detects missing name when no first, last, or company', () => {
    ...
    const issues = scanContactForIssues(contact, ['5551234567'], ['john@example.com']);
...
describe('scanContactForIssues — missing_email', () => {
  it('flags contact with no emails', () => {
    const contact = makeContact();
    const issues = scanContactForIssues(contact, ['5551234567'], []);
    const kinds = issues.map((i) => i.kind);
    expect(kinds).toContain('missing_email');
  });

  it('does not flag missing_email when email is set', () => {
    const contact = makeContact();
    const issues = scanContactForIssues(contact, ['5551234567'], ['john@example.com']);
    const kinds = issues.map((i) => i.kind);
    expect(kinds).not.toContain('missing_email');
  });
});
...
describe('scanContactForIssues — missing_phone', () => {
  it('flags contact with no phones', () => {
    const contact = makeContact();
    const issues = scanContactForIssues(contact, [], ['john@example.com']);
    const kinds = issues.map((i) => i.kind);
...
describe('scanContactForIssues — phone standardization', () => {
  it('flags a 10-digit number as missing country code', () => {
    const contact = makeContact();
    const issues = scanContactForIssues(contact, ['(555) 123-4567'], ['john@example.com']);
    const kinds = issues.map((i) => i.kind);
  ...
  it('flags duplicate phone numbers after normalization', () => {
    const contact = makeContact();
    const issues = scanContactForIssues(contact, ['5551234567', '(555) 123-4567'], ['john@example.com']);
    const kinds = issues.map((i) => i.kind);
...
describe('scanContactForIssues — clean contact', () => {
  it('returns no issues for a well-formed contact', () => {
    ...
    const issues = scanContactForIssues(contact, ['+1 (555) 123-4567'], ['john@example.com']);
